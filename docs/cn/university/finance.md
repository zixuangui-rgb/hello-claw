# 🧪 龙虾大学：金融简报实战（从信息碎片到决策支持）

> **适用场景**：每天花 2 小时看财经新闻，但关键信息总被淹没；想追踪自选股，但财报/宏观数据总错过；团队晨会材料临时拼凑，缺乏深度。

这篇文章不是简单的"金融新闻聚合"教程，而是一套**面向投资决策的智能信息处理系统**。它融合了 OpenClaw 的三大核心能力：**多源信息聚合**（Tavily + Summarize）、**结构化输出**（LLM 推理）、**自动化分发**（Cron + 飞书），让你从"信息消费者"变成"决策支持者"。

---

## 1. 你将得到什么（真实场景价值）

跑通后，你会拥有一个**24 小时在线的金融助理**：

### 场景 1：开盘前 30 分钟，手机收到结构化简报
- **问题**：开盘前手忙脚乱查新闻、看日历、做判断，容易遗漏关键信息
- **解决**：每天 9:00 自动推送"盘前简报"，包含：市场动态、自选股新闻、今日事件、风险提示、待复核项

### 场景 2：财报季不再手忙脚乱
- **问题**：财报季每天几十家公司发布财报，想看但看不过来
- **解决**：自定义财报监控规则，只提取"营收、利润、指引、异常项"四要素，自动推送高亮

### 场景 3：宏观数据发布日，提前 1 小时准备
- **问题**：非农、CPI、美联储会议等数据发布时，临时查背景资料，影响判断
- **解决**：提前 1 小时推送"数据前瞻"，包含：预期值、历史值、影响标的、历史规律

### 场景 4：团队晨会材料，每周五自动生成
- **问题**：每周五下午临时拼凑周报，质量参差不齐
- **解决**：每周五 16:00 自动生成"周度市场回顾"，包含：市场表现、自选股表现、事件复盘、下周展望
...
> **重要提示**：这 5 个技能已经足够支撑一套生产级金融工作流。金融信息处理对**准确性**要求极高，技能越多，误判风险越高。
...
这套工作流的核心价值不是"聚合金融信息"，而是**建立一种新的决策支持系统**：

- **早上 8:30**：查看盘前简报，准备开盘
- **下午 15:30**：查看盘后总结，复盘当日
- **每周五**：看周报，优化策略
- **每月 1 号**：回顾月度表现，调整组合

**记住**：龙虾不是替代你思考，而是帮你**把时间花在最重要的事情上**——信息整理、逻辑推理、决策判断。
