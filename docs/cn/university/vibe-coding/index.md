# 🧪 龙虾大学：Vibe Coding 实战（言出码随，对话即开发）

> **适用场景**：通勤路上突然想到一个 bug 修复思路，但笔记本不在身边；周末在咖啡厅想给开源项目加个小功能；团队需要快速原型验证，但开发者都在外面。**你不需要打开电脑，不需要写一行代码——只需要用自然语言描述你想要什么。**

Vibe Coding 的核心理念是**用对话代替编码**：你只负责"想清楚要做什么"，剩下的交给 AI。OpenClaw 负责接收飞书消息和通知用户（编排层），Claude Code 负责理解仓库、编辑代码、运行测试、创建 PR（编码引擎层）。你在飞书里用自然语言描述需求，就能完成"提需求 → 写代码 → 提 PR → 审查 → 合并"全流程。

---

## 1. 你将得到什么（真实场景价值）

跑通后，你会拥有一个**随叫随到的编码 Agent**：
