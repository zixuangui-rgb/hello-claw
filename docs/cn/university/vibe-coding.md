# 🧪 龙虾大学：Vibe Coding 实战（手机写代码，全程不开电脑）

> **适用场景**：通勤路上突然想到一个 bug 修复思路，但笔记本不在身边；周末在咖啡厅想给开源项目加个小功能，只带了手机；团队需要快速原型验证，但开发者都在外面。

这篇文章不是简单的"用手机发代码"教程，而是一套**从需求到合并的完整开发闭环**。OpenClaw 负责接收飞书消息和通知用户（编排层），Claude Code 负责理解仓库、编辑代码、运行测试、创建 PR（编码引擎层）。你用手机飞书聊天就能完成"提需求 → 写代码 → 提 PR → 审查 → 合并"全流程。

---

## 1. 你将得到什么（真实场景价值）

跑通后，你会拥有一个**随身携带的编码助手**：

### 场景 1：通勤路上修 Bug
- **问题**：发现线上 bug，笔记本不在身边，只能记个备忘等回家再修
- **解决**：在飞书跟龙虾说"修复 user.ts 第 42 行的空指针问题"，龙虾调用 Claude Code 自动新建分支、修改代码、跑测试、提交 PR，GitHub Copilot 自动审查，你在手机上确认合并

### 场景 2：Copilot 审查反馈闭环
- **问题**：Copilot 审查发现代码问题，传统流程需要打开电脑修改
...
[手机飞书] ←→ [OpenClaw Server (编排层)]
              ...
              [本地 Repo Workspace]
                     ↓
              [GitHub PR] → GitHub Copilot 自动审查
                     ↓
              [OpenClaw] → 飞书推送 PR 链接 + 代码修改摘要
                     ↓
              用户在手机上决策 → 修改/合并
```
...
手机上描述复杂需求，Claude Code 会自动分析依赖关系并分步执行：
...
Vibe Coding 的核心价值不是"用手机写代码"，而是**把开发者从繁琐的工具操作中解放出来**：

- **通勤路上**：飞书发一句需求，Claude Code 创建分支、写代码、跑测试、提 PR
- **午休时间**：看飞书推送的 Copilot 审查结果，回复"按建议修改"或"合并"
- **周末咖啡厅**：手机上完成一个完整的功能开发闭环
...
**记住**：Vibe Coding 不是偷懒，而是把编码回归本质——**想清楚要做什么，剩下的交给工具**。你不是在"用手机写代码"，而是在用手机**指挥一个专业的编码 Agent 团队**帮你把想法变成可审查的 PR。

### 更多可能：这个场景还能怎么玩？

本教程只展示了 Vibe Coding 的一种基础形态。基于同样的架构，你完全可以拓展出更多玩法，比如：

1. **开源贡献加速器**：用手机浏览 GitHub Trending，看到感兴趣的项目直接在飞书里说"给这个项目的 README 翻译成中文并提 PR"
...
欢迎大家在实践中探索新的玩法，并贡献到[龙虾大学](https://github.com/datawhalechina/hello-claw)！

---

**下一步**：[龙虾大学：首页](/cn/university/)
