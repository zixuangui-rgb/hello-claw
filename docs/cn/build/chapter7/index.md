# 第七章 轻量化方案：当 Agent 开始"瘦身"

> **核心问题**：OpenClaw 有 50 万行代码，但一个 Agent 最少需要多少行才能工作？
...
🌡️ **场景一：树莓派上跑 Agent**
你想在 30 块钱的树莓派上搭个智能家居助手。512MB 内存，OpenClaw 连启动都困难。

📚 **场景二：学习原理**
你想搞懂 Agent 到底怎么工作的。面对 50 万行代码，就像试图通过研究整架波音 747 来理解飞机原理——太复杂了。

✅ **场景三：简单需求**
你就想让 Agent 每天提醒一次喝水、每周整理一次周报。不需要 20 个渠道，不需要复杂的企业功能。
...
**① 单进程替代分布式**
OpenClaw 有 Gateway、Runtime、工具系统，各自独立运行，通过网络通信。这很灵活，但也复杂。
NanoClaw 就一个大循环：轮询 SQLite 数据库 → 发现新消息 → 启动容器处理 → 返回结果。没有网络通信，没有分布式协调。

**② 容器隔离替代权限配置**
OpenClaw 靠配置文件控制"你能访问哪个文件夹"。这是应用层权限，配置复杂，还可能出漏洞。
...
| 功能 | OpenClaw | NanoClaw | Nanobot | ZeroClaw |
|------|----------|----------|---------|----------|
| **代码量** | ~500k | ~7k | ~4k | 未知 |
| **架构** | 分布式多进程 | 单进程 | 单进程 | 单二进制 |
| **安全模型** | 应用层权限 | 容器隔离 | 应用层+沙箱 | 默认拒绝 |
| **硬件支持** | ❌ | ❌ | ❌ | ✅ |
| **安装** | npm | git+claude | pip | cargo |
