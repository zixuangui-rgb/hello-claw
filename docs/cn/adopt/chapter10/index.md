...
│  ┌───────────────────┐   ┌───────────────────┐   │
│  │    VM1（桌面版）     │   │  VM2（最小化 CLI）  │   │
│  │  Debian/Ubuntu GUI  │   │   CLI Proxy API   │   │
│  │                     │   │                   │   │
│  │  • OpenClaw 完整运行 │◄──┤  • 真实 API Key   │   │
│  │  • 浏览器自动化      │ 内网│  • 密钥中转代理    │──►│ 模型 API
│  │  • 能力不打折        │   │  • 无其他服务      │   │
│  │                     │   │                   │   │
│  │  ⚠ 无真实 API Key  │   │  ✅ 密钥唯一存放   │   │
│  └───────────────────┘   └───────────────────┘   │
...
┌─────────────────────────────────────────────────┐
│              不可信区域（外部世界）                │
│   WhatsApp / Telegram / Discord / 网页 / 邮件    │
└───────────────────────┬─────────────────────────┘
                        ▼
┌─────────────────────────────────────────────────┐
│         信任边界 1：渠道接入控制                   │
│   设备配对（30秒有效期）+ AllowFrom 白名单         │
...
│   每个 Agent 独立的工具策略 + 对话日志             │
└───────────────────────┬─────────────────────────┘
                        ▼
┌─────────────────────────────────────────────────┐
│         信任边界 3：工具执行                       │
│   Docker 沙箱 / 主机执行（需审批）                 │
│   节点远程执行 + SSRF 防护（DNS 锁定 + IP 拦截）  │
└───────────────────────┬─────────────────────────┘
                        ▼
┌─────────────────────────────────────────────────┐
│         信任边界 4：外部内容                       │
...
┌─────────────────────────────────────────────────┐
│         信任边界 5：供应链                         │
│   ClawHub 技能发布（semver + SKILL.md 必需）      │
│   模式审核 + VirusTotal 扫描 + GitHub 账号年龄验证 │
└─────────────────────────────────────────────────┘
```
