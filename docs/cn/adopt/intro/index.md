...
**第一部分：领养 Claw**——使用篇，11 章 + 7 附录，两层结构，从零基础到完整配置：

- **🔵 安装**（第 1～3 章）——第 1 章下载 AutoClaw 桌面客户端，5 分钟零门槛体验；第 2 章手动安装 OpenClaw，完全掌控配置和数据；第 3 章配置向导，完成首次设置。
- **🟢 场景配置**（第 4～11 章）——按功能模块逐步配置：聊天平台接入（Ch4）、模型管理（Ch5）、智能体管理（Ch6）、工具与定时任务（Ch7）、网关运维（Ch8）、远程访问与网络（Ch9）、安全防护与威胁模型（Ch10）、Web 界面与客户端（Ch11）。装好后按需选读，不必按顺序。
...
**下一步**：[第一章 AutoClaw 一键安装](/cn/adopt/chapter1/)
