...
这种分层设计让 OpenClaw 既灵活又可控。你可以通过修改 SOUL.md 来定制 Agent 的性格和行为准则，通过 USER.md 和 MEMORY.md 让它认识你并记住你的偏好，通过安装不同的技能来扩展它的能力。详细介绍见[第九章 个性化定制](/cn/adopt/chapter9/)。
...
**云端托管 / Docker 部署**：详见[第十章 生产环境部署](/cn/adopt/chapter10/)和[附录 E 云服务部署指南](/cn/appendix/appendix-e)。

### 6.2 学习路径

本教程分为五个层级，循序渐进：

**🔵 第一层：光速上手**（第一章）——下载 AutoClaw，5 分钟拥有 AI 助手，零门槛体验。

**🟢 第二层：手动安装**（第二～三章）——手动安装 OpenClaw、接入聊天平台（QQ/飞书/Telegram），完全掌控配置和数据。

**🟡 第三层：日常使用**（第四～六章）——命令行与配置、定时任务、技能系统入门，让龙虾真正为你干活。

**🟠 第四层：高级配置**（第七～九章）——多平台与外部服务集成、多模型与成本优化、个性化定制（工作区与记忆系统）。

**🔴 第五层：高级场景**（第十～十二章）——生产环境部署、开发者效率提升、故障排查与维护。

**第二部分：构建 Claw**——从零实现一个简化版 AI Agent，深入理解 Agent 的工作原理。
