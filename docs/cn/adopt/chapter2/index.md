...
部署页面会显示完整的 OpenClaw 原生接入流程。打开终端，按照指引依次执行以下三条命令：
...
> 注意：Token 格式为 `AppID:AppSecret`，中间用英文冒号分隔。例如：`"1903127255:tqkcPCOnbQCuclyY"`
...
配置完成后，回到浏览器的部署页面，点击"扫描聊天"按钮，用手机 QQ 扫码即可找到你的机器人。发送一条消息试试：
